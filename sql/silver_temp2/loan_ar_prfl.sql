...
-- Target: loan_ar_prfl (Silver Layer) - Compliant with PG_Source_PoC.xlsx mapping
-- ====================================================================
...
WITH live_prfl AS (
    SELECT 
        ...
        TRY_CAST(COALESCE(d.SO_LAN_CO_CAU, c.FIELD_VAL) AS DECIMAL(20,0)) AS NBR_OF_DBT_RSTC,
        ...
        CASE WHEN e.CT_KHUYEN_MAI_CHO_VAY IS NULL AND i.KHOI = 'INDIV' THEN CAST(f.MB_LD_TYPE AS STRING) ELSE CAST(e.CT_KHUYEN_MAI_CHO_VAY AS STRING) END AS PROM_PRGM,
        CASE WHEN e.CTKM2 IS NULL AND i.KHOI <> 'INDIV' THEN CAST(f.MB_LD_TYPE AS STRING) END AS PROM_PRGM_2,
        g.CO_CAU_NO                                            AS DBT_RSTC_TP_CODE,
        COALESCE(TRY_TO_DATE(g.NGAY_HET_COVID19, 'dd/MM/yyyy'), TRY_TO_DATE(g.NGAY_HET_COVID19, 'yyyyMMdd'), TRY_TO_DATE(g.NGAY_HET_COVID19, 'yyyy-MM-dd')) AS COVID19_RSTC_COMPL_DT,
...
his_prfl AS (
    SELECT 
        sha2(concat('T24_LD_LOANS_AND_DEPOSITS', SPLIT(a.RECID, ';')[0]), 256) AS AR_ID,
        a.INT_LIQ_ACCT                                           AS INT_LQD_AR_CODE,
        sha2(concat('T24_ACCOUNT', a.INT_LIQ_ACCT), 256)          AS INT_LQD_AR_ID,
        a.PRIN_LIQ_ACCT                                          AS PNP_LQD_AR_CODE,
        sha2(concat('T24_ACCOUNT', a.PRIN_LIQ_ACCT), 256)         AS PNP_LQD_AR_ID,
        a.RECORD_STATUS                                          AS AR_LCS_TP_CODE,
        a.CO_CODE                                                AS OU_CODE,
        b.UDF_VALUE                                              AS MAND_REPYMT,
        TRY_CAST(COALESCE(d.SO_LAN_CO_CAU, c.FIELD_VAL) AS DECIMAL(20,0)) AS NBR_OF_DBT_RSTC,
        c1.FIELD_VAL                                             AS SCR_TP,
        ...
        d.TP_PP_QL_KHOAN_VAY                                     AS MGT_MGR,
        d.CAP_PHE_DUYET_GNTL                                     AS APRV_AHR,
        CAST(NULL AS STRING)                                     AS CHKER_OFCR,
        CAST(NULL AS STRING)                                     AS MAKER_OFCR,
        CAST(NULL AS STRING)                                     AS REFRRER_OFCR,
        CASE WHEN e.CT_KHUYEN_MAI_CHO_VAY IS NULL AND i.KHOI = 'INDIV' THEN CAST(f.MB_LD_TYPE AS STRING) ELSE CAST(e.CT_KHUYEN_MAI_CHO_VAY AS STRING) END AS PROM_PRGM,
        CASE WHEN e.CTKM2 IS NULL AND i.KHOI <> 'INDIV' THEN CAST(f.MB_LD_TYPE AS STRING) END AS PROM_PRGM_2,
        g.CO_CAU_NO                                              AS DBT_RSTC_TP_CODE,
        COALESCE(TRY_TO_DATE(g.NGAY_HET_COVID19, 'dd/MM/yyyy'), TRY_TO_DATE(g.NGAY_HET_COVID19, 'yyyyMMdd'), TRY_TO_DATE(g.NGAY_HET_COVID19, 'yyyy-MM-dd')) AS COVID19_RSTC_COMPL_DT,
        ...
        TO_DATE('9999-12-31', 'yyyy-MM-dd')                      AS SYS_EXP_DT,
        current_timestamp()                                       AS SYS_UDT_DT
    FROM bz_t24core_ld_loans_and_deposits_his a
    LEFT JOIN bz_ebanking_col_udf_value b ON b.COL_REF = a.ACC_FCC AND b.FUNCTION_CODE = 'CONTRACT_INFOR' AND a.RECID LIKE 'LD%' AND b.UDF_ID = 'NHANNO_BATBUOC'
    LEFT JOIN bz_flexbo_pgb_contract_udf_map c ON c.CONTRACT_REF_NO = a.ACC_FCC AND c.FIELD_NAME = 'SO LAN CO CAU'
    LEFT JOIN bz_flexbo_pgb_contract_udf_map c1 ON c1.CONTRACT_REF_NO = a.ACC_FCC AND c1.FIELD_NAME = 'LOAI CHUNG KHOAN'
    LEFT JOIN bz_flexbo_pgb_los_contract_fields_tdate d ON d.CONTRACT_REF_NO = SPLIT(a.RECID, ';')[0] AND d.MOV_DATE = COALESCE(a.ORIG_VAL_DATE, a.VALUE_DATE)
    LEFT JOIN bz_flexbo_pgbld_contract_udfield_hist e ON e.CONTRACT_REF_NO = a.ACC_FCC
    LEFT JOIN bz_t24core_mb_mg_saving_multi f ON f.LD_ID = SPLIT(a.RECID, ';')[0]
    LEFT JOIN bz_flexbo_pgbld_rt_contract_udfield_hist g ON g.CONTRACT_REF_NO = a.ACC_FCC
    LEFT JOIN (SELECT DISTINCT TRANS_REF, SBV_CODE FROM bz_source_saoke_mvmt WHERE SBV_CODE LIKE '39%') h ON h.TRANS_REF = SPLIT(a.RECID, ';')[0]
    LEFT JOIN bz_t24core_customer i ON i.RECID = a.CUSTOMER_ID
    WHERE NOT EXISTS (
...
pd_prfl AS (
    SELECT 
        ...
        d.REPAYMENT_ACCT                                        AS INT_LQD_AR_CODE,
        sha2(concat('T24_ACCOUNT', d.REPAYMENT_ACCT), 256)      AS INT_LQD_AR_ID,
        CAST(NULL AS STRING)                                    AS PNP_LQD_AR_CODE,
        CAST(NULL AS STRING)                                    AS PNP_LQD_AR_ID,
        d.STATUS                                                AS AR_LCS_TP_CODE,
        d1.CO_CODE                                              AS OU_CODE,
        CAST(NULL AS STRING)                                    AS MAND_REPYMT,
        CAST(NULL AS DECIMAL(20,0))                             AS NBR_OF_DBT_RSTC,
        ...
        d.AUTHORISER                                            AS CHKER_OFCR,
        d.INPUTTER                                              AS MAKER_OFCR,
        CAST(NULL AS STRING)                                    AS REFRRER_OFCR,
        ...
        d.CREDIT_STATUS                                         AS FNC_ST_CODE,
        current_date()                                          AS SYS_EFF_DT,
        TO_DATE('9999-12-31', 'yyyy-MM-dd')                     AS SYS_EXP_DT,
        current_timestamp()                                      AS SYS_UDT_DT

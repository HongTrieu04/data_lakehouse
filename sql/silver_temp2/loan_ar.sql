...
-- Target: demo.default.loan_ar (Silver Layer)
-- ====================================================================

WITH live_branch AS (
    SELECT 
        sha256(concat('T24_LD_LOANS_AND_DEPOSITS', ld.RECID)) AS AR_ID,
        ld.RECID                                             AS AR_CODE,
        ...
        sha256(concat('T24_CUSTOMER', ld.CUSTOMER_ID))       AS CST_ID,
        ...
        sha256(concat('T24_LIMIT', ld.LIMIT_REFERENCE))      AS LMT_ID,
    ...
    FROM demo.default.bz_t24core_ld_loans_and_deposits ld
    LEFT JOIN demo.default.bz_flexbo_pgb_ldtb_contract_master m ON ld.ACC_FCC = m.CONTRACT_REF_NO
    LEFT JOIN demo.default.bz_flexbo_pgb_los_contract_fields_tdate td ON ld.RECID = td.CONTRACT_REF_NO AND td.MOV_DATE = COALESCE(ld.ORIG_VAL_DATE, ld.VALUE_DATE)
    LEFT JOIN demo.default.bz_los_app_loan_disbursement ld_los ON ld_los.EXT_REF_NO_1 = ld.RECID AND ld_los.AMND_STATE = 'F' AND ld_los.STATUS IN ('A','U') AND ld_los.BOOKING_DATE = COALESCE(ld.ORIG_VAL_DATE, ld.VALUE_DATE)
    LEFT JOIN demo.default.bz_los_app_facility los ON ld_los.FACILITY_ID = los.ID
    LEFT JOIN demo.default.bz_los_app_product pd_los ON los.PRODUCT_ID = pd_los.ID
    LEFT JOIN demo.default.bz_t24core_customer i ON i.RECID = ld.CUSTOMER_ID
),
his_branch AS (
    SELECT 
        sha256(concat('T24_LD_LOANS_AND_DEPOSITS', SPLIT(ldh.RECID, ';')[0])) AS AR_ID,
        ...
        sha256(concat('T24_CUSTOMER', ldh.CUSTOMER_ID))      AS CST_ID,
        ...
        sha256(concat('T24_LIMIT', ldh.LIMIT_REFERENCE))     AS LMT_ID,
        ...
        TO_DATE('9999-12-31', 'yyyy-MM-dd')                 AS SYS_EXP_DT,
        current_timestamp()                                  AS SYS_UDT_DT
    FROM demo.default.bz_t24core_ld_loans_and_deposits_his ldh
    WHERE NOT EXISTS (

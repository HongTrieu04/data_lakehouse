...
def ingest_oracle_to_bronze(etl_date: str = "2026-08-06", target_table: str = None):
    ...
    for item in tables_to_process:
        ...
        try:
            ...
            table_folder = item["table"]
            write_iceberg_table(df_bronze, bucket="bronze", schema_name=schema_folder, table_name=table_folder, mode="append")

            print(f"[SUCCESS] Successfully ingested {src_table} -> Iceberg bronze/{schema_folder}/{table_folder.upper()}")

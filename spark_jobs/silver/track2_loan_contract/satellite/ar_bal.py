...
def run_ar_bal(etl_date: str = "2026-08-06"):
    ...
    # Save output parquet & iceberg table and register temporary view for downstream tasks
    target_path = f"s3a://silver/ar_bal/{etl_date}/"
    write_parquet(df_result, target_path, mode="overwrite")
    write_iceberg_table(df_result, "ar_bal", mode="overwrite")
    df_result.createOrReplaceTempView("ar_bal")
    
    print(f"[SILVER SATELLITE] Completed AR_BAL - {df_result.count()} records written")
